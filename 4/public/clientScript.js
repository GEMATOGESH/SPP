...
socket.on('newtask', function(status) {
    if (status == 401) {
        fourZeroOne();
    }
    else {
...
{
	socket.emit('mark', JSON.stringify({"token": token, "id": id, "status": status}));
}
...
async function addTask() 
{
	...
	file = document.getElementById('btnFile').files;

    socket.emit('newtask', JSON.stringify({"date": date.value, "time": time.value, "task": task.value, "file": file[0].name, "token": token}));
...
async function auth() {
    ...
    socket.emit('auth', JSON.stringify( { "login": login, "pass": pass } ));
}
...
async function reg() {
	...
	socket.emit('reg', JSON.stringify({"login": login, "pass": pass}));
}
